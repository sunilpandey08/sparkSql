...
object xmlData {
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder.master("local[*]").appName("xmlData").enableHiveSupport().getOrCreate()
    val sc = spark.sparkContext
    val conf = new SparkConf().setAppName("xmlData").setMaster("local[*]")
    //    val sc = new SparkContext(conf)
    //val sqlContext = new org.apache.spark.sql.SQLContext(sc)
    val sqlContext = spark.sqlContext
    import spark.implicits._
    import spark.sql
    val hivexmltab = args(0)
    val xmlData = "file:///home/hadoop/ravi_kiran_trans00005.xml"
    val xmlDF = spark.read.format("xml").option("rowTag","Transaction").load(xmlData)
    xmlDF.printSchema()
    xmlDF.show()
